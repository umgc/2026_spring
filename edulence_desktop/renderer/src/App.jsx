...
function ThemeOption({ label, subtitle, selected, onClick }) {
  return (
    <button type="button" className={`list-tile ${selected ? 'selected' : ''}`} onClick={onClick}>
...
export default function App() {
  ...
  const refreshRecentFiles = async () => {
    const result = await invokeSafe('Load recent files', () => window.desktop.app.getRecentFiles());
    if (result?.ok) {
  ...
  useEffect(() => {
    let cancelled = false;
    invokeSafe('Load desktop state', () => window.desktop.state.get()).then((result) => {
      if (cancelled) return;
      ...
      if (prefs) {
        if (typeof prefs.themeMode === 'string') setThemeMode(prefs.themeMode);
        if (typeof prefs.leftHanded === 'boolean') setLeftHanded(prefs.leftHanded);
      ...
      <div className="auth-shell">
        ...
        <main className="auth-layout">
          <section className="auth-intro">
            ...
            <p>
              Your personalized learning hub. Sign in to continue or create an account to get
              started.
            </p>
          </section>
          ...
          <section className="auth-card" aria-label="Authentication actions">
            ...
            <p>
              Save progress, sync across devices, and unlock tailored study insights across mobile
              and desktop.
            </p>
            ...
            <p className="auth-footnote">By continuing you agree to our Terms & Privacy.</p>
          </section>
        </main>
      </div>
    );
  ...
  return (
    <div className={`flutter-shell ${leftHanded ? 'left-handed' : 'right-handed'}`}>
      <header className="app-bar" role="banner">
        <div className="app-bar-leading">
          <button type="button" onClick={navigateBack} disabled={historyIndex <= 0} aria-label="Back">
            Back
          </button>
        </div>
        <h1 className="app-bar-title">{route === 'home' ? 'EduLense' : ROUTE_LABELS[route]}</h1>
        <div className="app-bar-actions">
          <button type="button" onClick={navigateForward} disabled={historyIndex >= history.length - 1}>
            Forward
          </button>
      ...
      <div className="desktop-utility-rail" aria-label="Desktop controls">
        <button type="button" className="rail-button primary" onClick={handleNew}>New</button>
        <button type="button" className="rail-button" onClick={() => handleSave(false)}>
          Save{dirty ? ' *' : ''}
        </button>
        <button type="button" className="rail-button" onClick={() => window.desktop.app.toggleTrayWindow()}>
          Tray
        </button>
        <button type="button" className="rail-button" onClick={() => setShowShortcuts(true)}>
          Keys
        </button>
      </div>
          ...
          <div className="screen-stack">
            <section className="m-card hero-card">
              ...
              <div className="hero-actions">
                <button type="button" className="filled" onClick={handleOpen}>Open Learning File</button>
                <button type="button" onClick={handleOpenFolder}>Open Folder</button>
                <button type="button" className="outlined" onClick={handleNew}>Create Note</button>
              </div>
            </section>
        ...
        {route === 'files' && (
          <div className="screen-stack">
            <section className="m-card">
              <h2>Explore</h2>
              <p className="tile-subtitle">Flutter-like list cards adapted for desktop files and topics.</p>
              <label className="field-label" htmlFor="file-filter">Search</label>
              <input
                id="file-filter"
                value={filter}
            ...
            <section>
              <div className="list-stack">
                {topicTiles.map((tile) => (
                  <button type="button" key={tile.title} className="m-card list-tile" onClick={() => setStatus(`Opened topic ${tile.title}`)}>
            ...
            <section>
              ...
              <div className="list-stack">
                {filteredRecent.length === 0 ? (
                  <div className="m-card empty-state">No recent files yet. Open or save a file to populate this list.</div>
                ) : (
            ...
            {openedFolder && (
              <section>
                ...
                <div className="m-card">
                  <div className="tile-subtitle ellipsis">{openedFolder.path}</div>
                </div>
                <div className="list-stack">
                      ...
                      <button
                        ...
                        onClick={async () => {
                          if (entry.isDirectory) {
                            const next = await window.desktop.file.listDir(entry.path);
                            if (next?.ok) {
                              setOpenedFolder({ path: next.folderPath, entries: next.entries || [] });
                        ...
                        title={entry.path}
                      >
                        <span className={`tile-leading ${entry.isDirectory ? 'info' : ''}`}>
                          {entry.isDirectory ? 'Dir' : 'File'}
                        </span>
                        <span className="tile-copy">
                          <span className="tile-title">{entry.name}</span>
                          <span className="tile-subtitle ellipsis">{entry.path}</span>
                        </span>
                        <span className="row-chevron">›</span>
                      </button>
                    ))
        ...
        {route === 'notes' && (
          <div className="screen-stack">
            ...
            <section className="list-stack">
              <button type="button" className="m-card list-tile" onClick={() => setStatus('Opened Completed Lessons')}>
                <span className="tile-leading success">Done</span>
                <span className="tile-copy">
                  <span className="tile-title">Completed Lessons</span>
                  <span className="tile-subtitle">View your learning history</span>
                </span>
                <span className="row-chevron">›</span>
              </button>
              <button type="button" className="m-card list-tile" onClick={() => setStatus('Opened Learning Goals')}>
                <span className="tile-leading">Goal</span>
                <span className="tile-copy">
                  <span className="tile-title">Learning Goals</span>
                  <span className="tile-subtitle">Track your weekly targets</span>
                </span>
                <span className="row-chevron">›</span>
              </button>
              <button
              ...
              >
                ...
                <span className="tile-copy">
                  ...
                  <span className="tile-subtitle">Return to the authentication screen</span>
                </span>
                <span className="row-chevron">›</span>
              </button>
            </section>
            ...
            <section className="m-card editor-card">
              <div className="editor-header">
                <div>
                  <h3 className="section-title compact">Desktop Notes Workspace</h3>
                  <div className="tile-subtitle">{currentFilePath || 'No file selected'}</div>
                </div>
                <div className={`status-pill ${dirty ? 'warning' : 'success'}`}>{dirty ? 'Unsaved' : 'Saved'}</div>
              </div>
              <textarea
                ref={editorRef}
                value={content}
              ...
              <div className="editor-actions">
                <button type="button" className="filled" onClick={() => handleSave(false)}>Save</button>
                <button type="button" className="outlined" onClick={() => handleSave(true)}>Save As</button>
                <button type="button" onClick={handleOpen}>Open</button>
                <button
                  type="button"
                  onClick={() => currentFilePath && window.desktop.app.showItemInFolder(currentFilePath)}
            ...
            <section>
              ...
              <div className="m-card card-stack">
                ...
                <ThemeOption
                  ...
                  selected={false}
                  onClick={() => setStatus('System theme mapping not yet implemented in main process')}
                />
              </div>
            </section>
            ...
            <section>
              ...
              <div className="m-card about-card">
                ...
                <p>
                  ...
                  adapted here to a desktop Electron experience.
                </p>
                ...
                <div className="updater-panel">
                  ...
                  <div className="tile-subtitle">
                    {updaterStatus.message || 'Updater status unavailable'} ({updaterStatus.state})
                  </div>
                  <div className="editor-actions">
                    <button type="button" onClick={() => window.desktop.updater.check()}>
                      Check
                    </button>
                    <button type="button" onClick={() => window.desktop.updater.download()}>
                      Download
                    </button>
                    <button type="button" onClick={() => window.desktop.updater.install()}>
                      Install & Restart
                    </button>
                  </div>
                </div>
      ...
      <footer className="status-bar" role="status" aria-live="polite">
        <span>{status}</span>
        <span>{dirty ? `Modified · ${formatTitle(currentFilePath)}` : 'Synced'}</span>
      </footer>

...
await user.click(within(banner).getByRole("button", { name: /^settings$/i }));
  ...
  test("courses tabs support keyboard navigation", async () => {
    await renderAuthenticatedApp();

    fireEvent.keyDown(window, { key: "2", ctrlKey: true });
    await screen.findByRole("heading", { name: /^courses$/i });

    const allTab = screen.getByRole("tab", { name: /all courses/i });
    allTab.focus();

    fireEvent.keyDown(allTab, { key: "ArrowRight" });
    expect(screen.getByRole("tab", { name: /in progress/i })).toHaveFocus();

    fireEvent.keyDown(document.activeElement, { key: "End" });
    expect(screen.getByRole("tab", { name: /favorites/i })).toHaveFocus();

    fireEvent.keyDown(document.activeElement, { key: "Home" });
    expect(screen.getByRole("tab", { name: /all courses/i })).toHaveFocus();
  });
  ...
  test("notes filter buttons work", async () => {
    const { user } = await renderAuthenticatedApp();
    ...
    expect(screen.getByRole("button", { name: /calculus lecture 15/i })).toBeInTheDocument();
  ...
  test("open note button calls desktop file open", async () => {
    const { user, desktop } = await renderAuthenticatedApp();
    ...
    expect(desktop.file.open).toHaveBeenCalled();
  });
  ...
  test("calendar, progress, achievements, study sessions, and notifications pages render", async () => {
    const { user } = await renderAuthenticatedApp();
    const nav = screen.getByRole("navigation", { name: /main navigation tabs/i });

    await user.click(within(nav).getByRole("button", { name: /calendar, 3 items/i }));
    expect(await screen.findByRole("heading", { name: /^calendar$/i })).toBeInTheDocument();

    await user.click(within(nav).getByRole("button", { name: /^progress$/i }));
    expect(await screen.findByRole("heading", { name: /^progress$/i })).toBeInTheDocument();

    await user.click(within(nav).getByRole("button", { name: /^achievements$/i }));
    expect(await screen.findByRole("heading", { name: /^achievements$/i })).toBeInTheDocument();

    await user.click(within(nav).getByRole("button", { name: /^study sessions$/i }));
    expect(await screen.findByRole("heading", { name: /^study sessions$/i })).toBeInTheDocument();

    await user.click(within(nav).getByRole("button", { name: /notifications, 12 items/i }));
    expect(await screen.findByRole("heading", { name: /^notifications$/i })).toBeInTheDocument();
  });

  test("dashboard new course button navigates to courses", async () => {
    const { user } = await renderAuthenticatedApp();
  ...
  test("dashboard upcoming panel controls are present", async () => {
    await renderAuthenticatedApp();
    ...
    expect(screen.getByText(/calculus exam/i)).toBeInTheDocument();
    expect(screen.getByText(/physics lab report due/i)).toBeInTheDocument();
  });

  test("study groups button updates status text", async () => {
    const { user } = await renderAuthenticatedApp();

    await user.click(screen.getByRole("button", { name: /study groups/i }));
    expect(screen.getByRole("status")).toHaveTextContent(/study groups coming soon/i);
  });
});
...
const COURSES = [
  { id: "MATH-301", title: "Advanced Calculus", code: "MATH-301", instructor: "Dr. Jane Smith", credits: 3, progress: 75, status: "In Progress", schedule: "Mon, 9:00 AM", accent: "lavender" },
  { id: "PHYS-202", title: "Physics II", code: "PHYS-202", instructor: "Prof. John Davis", credits: 4, progress: 60, status: "In Progress", schedule: "Tue, 10:30 AM", accent: "mint" },
  { id: "CS-101", title: "Computer Science", code: "CS-101", instructor: "Dr. Sarah Johnson", credits: 3, progress: 85, status: "In Progress", schedule: "Wed, 2:00 PM", accent: "sand" },
  { id: "ENG-250", title: "English Literature", code: "ENG-250", instructor: "Prof. Liam Carter", credits: 3, progress: 45, status: "In Progress", schedule: "Thu, 1:00 PM", accent: "sky" },
  { id: "HIST-210", title: "World History", code: "HIST-210", instructor: "Dr. Elena Ruiz", credits: 3, progress: 20, status: "In Progress", schedule: "Fri, 11:00 AM", accent: "rose" },
  { id: "BIO-110", title: "Biology", code: "BIO-110", instructor: "Prof. Ava Nguyen", credits: 4, progress: 100, status: "Completed", schedule: "Self-paced", accent: "lime" },
  { id: "CHEM-115", title: "Chemistry I", code: "CHEM-115", instructor: "Dr. Noah Brown", credits: 4, progress: 35, status: "In Progress", schedule: "Tue, 8:00 AM", accent: "mint" },
  { id: "ART-205", title: "Modern Art", code: "ART-205", instructor: "Prof. Maya Lee", credits: 2, progress: 100, status: "Completed", schedule: "Fri, 3:00 PM", accent: "rose" },
];
...
const COURSE_TAB_OPTIONS = ["all", "in-progress", "completed", "favorites"];
const NOTE_FILTER_OPTIONS = ["all", "favorites", "recent"];
...
function CourseCard({ course, onOpen }) {
  ...
  return (
    <button
      ...
      aria-label={`${course.title}, ${course.code}, ${course.progress}% complete, ${stateLabel}`}
    >
      ...
      <div className="course__meta">
        ...
        <p className="course__sub">{course.instructor} · {course.credits} Credits</p>
        ...
        <div className="course__footer">
          <Pill tone={completed ? "success" : "neutral"}>{stateLabel}</Pill>
      ...
      <section className="dashboard-grid">
        <div className="panel">
          <div className="panel__header">
            <h2>My Courses</h2>
            <button type="button" className="link" onClick={onPrimaryAction}>View All →</button>
          </div>
        ...
        <aside className="panel panel--aside" aria-label="Upcoming work">
          <div className="panel__header">
            <h2>Upcoming</h2>
            <button type="button" className="btn btn--icon" aria-label="Open calendar">📅</button>
          </div>
          ...
          <button type="button" className="btn btn--ghost btn--full" aria-label="View calendar">View Calendar</button>
        </aside>
      </section>
    </div>
  );
...
function CoursesPage({ filter, setFilter, headingRef }) {
  ...
  const filteredCourses = useMemo(() => {
    ...
    if (activeTab === "favorites") list = list.filter((c) => ["MATH-301", "CS-101"].includes(c.id));
    if (!search) return list;
    return list.filter((course) => (`${course.title} ${course.code} ${course.instructor}`).toLowerCase().includes(search));
  }, [activeTab, filter]);
  ...
  const onTabKeyDown = (event, tab) => {
    const currentIndex = COURSE_TAB_OPTIONS.indexOf(tab);
    if (currentIndex === -1) return;
    let nextIndex = currentIndex;
    if (event.key === "ArrowRight") nextIndex = (currentIndex + 1) % COURSE_TAB_OPTIONS.length;
    else if (event.key === "ArrowLeft") nextIndex = (currentIndex - 1 + COURSE_TAB_OPTIONS.length) % COURSE_TAB_OPTIONS.length;
    else if (event.key === "Home") nextIndex = 0;
    else if (event.key === "End") nextIndex = COURSE_TAB_OPTIONS.length - 1;
    else return;
    event.preventDefault();
    selectTab(COURSE_TAB_OPTIONS[nextIndex]);
  };
  ...
  return (
    <div className="page">
      <header className="page__header">
        <div>
          <h1 ref={headingRef} tabIndex={-1}>Courses</h1>
          <p className="muted">Manage and track all your courses</p>
        </div>
        <button type="button" className="btn btn--primary"><span aria-hidden="true">📖</span>Add Course</button>
      </header>

      <div className="courses-toolbar" role="search">
        <label className="search">
          <span className="search__icon" aria-hidden="true">🔎</span>
          <input type="search" value={filter} onChange={(e) => setFilter(e.target.value)} placeholder="Search courses…" aria-label="Search courses" />
        </label>
        <div className="courses-toolbar__actions" role="toolbar" aria-label="Courses view options">
          <button type="button" className="btn btn--icon" aria-label="Filter">☰</button>
          <button type="button" className="btn btn--icon" aria-label="Grid view" aria-pressed="true">▦</button>
          <button type="button" className="btn btn--icon" aria-label="List view" aria-pressed="false">≡</button>
        </div>
      </div>

      <div className="tabs" role="tablist" aria-label="Course filters">
        {COURSE_TAB_OPTIONS.map((tab) => {
          const labels = {
            all: "All Courses",
          ...
          return (
            ...
            >
              {labels[tab]} {tab !== "favorites" ? <span className="tab__count">({counts[tab]})</span> : null}
            </button>
          );
        })}
      </div>

      <section className="course-grid" role="tabpanel" aria-labelledby={`course-tab-${activeTab}`} aria-label="All courses">
...
function NotesPage({
  ...
  }, [selectedNoteId, currentFilePath, dirty, content, selected.body, setContent]);

  const noteList = useMemo(() => {
    const q = noteSearch.trim().toLowerCase();
    let list = SAMPLE_NOTES;
    if (activeFilter === "favorites") list = SAMPLE_NOTES.filter((n) => n.id === "calc-15");
    if (activeFilter === "recent") list = SAMPLE_NOTES.slice(0, 1);
    if (!q) return list;
    return list.filter((n) => n.title.toLowerCase().includes(q) || n.course.toLowerCase().includes(q));
  }, [activeFilter, noteSearch]);

  return (
    <div className="notes-layout">
      <aside className="notes-sidebar" aria-label="Notes list">
        <div className="notes-sidebar__header">
          <h2 ref={headingRef} tabIndex={-1}>Notes</h2>
          <button type="button" className="btn btn--icon" onClick={onNew} aria-label="Create new note">＋</button>
        </div>
        <label className="search search--compact">
          <span className="search__icon" aria-hidden="true">🔎</span>
          <input type="search" value={noteSearch} onChange={(e) => setNoteSearch(e.target.value)} placeholder="Search notes…" aria-label="Search notes" />
        </label>

        <nav className="notes-filters" aria-label="Note filters">
          {NOTE_FILTER_OPTIONS.map((filterName) => {
            const labels = { all: "All Notes", favorites: "Favorites", recent: "Recent" };
            const badges = { all: 42, favorites: 8, recent: null };
            return (
              <button
                key={filterName}
                type="button"
                className={cn("notes-filter", activeFilter === filterName && "is-active")}
                aria-pressed={activeFilter === filterName}
                onClick={() => setActiveFilter(filterName)}
              >
                {labels[filterName]} {badges[filterName] ? <span className="badge">{badges[filterName]}</span> : null}
              </button>
            );
          })}
        </nav>
      ...
      <section className="notes-editor" aria-label="Notes editor">
        <div className="notes-toolbar" role="toolbar" aria-label="Note actions">
          <div className="notes-toolbar__actions">
            <button type="button" className="btn btn--icon" onClick={onOpen} aria-label="Open note">📂</button>
            <button type="button" className="btn btn--icon" onClick={() => onSave(false)} aria-label="Save note" disabled={!dirty && !currentFilePath}>💾</button>
            <button type="button" className="btn btn--icon" onClick={onSaveAs} aria-label="Save note as">⤓</button>
          </div>
          <div className="notes-toolbar__format" aria-label="Formatting controls (visual only)">
            <button type="button" className="btn btn--icon" aria-label="Bold"><strong>B</strong></button>
            <button type="button" className="btn btn--icon" aria-label="Italic"><em>I</em></button>
            <button type="button" className="btn btn--icon" aria-label="Underline"><span style={{ textDecoration: "underline" }}>U</span></button>
            <span className="toolbar-divider" aria-hidden="true" />
            <button type="button" className="btn btn--icon" aria-label="Bulleted list">•</button>
            <button type="button" className="btn btn--icon" aria-label="Numbered list">1.</button>
            <span className="toolbar-divider" aria-hidden="true" />
            <button type="button" className="btn btn--icon" aria-label="Link">🔗</button>
          </div>
        </div>
  ...
  className="editor"
  spellCheck
  aria-label="Note editor content"
          />
        </section>
...
function PlaceholderPage({ title, children, headingRef }) {
  return (
    <div className="page">
      ...
      <div className="panel"><div className="panel__body">{children}</div></div>
    </div>
  );
}
...
function SettingsPage({
  ...
  const onThemeKeyDown = (event, currentTheme) => {
    const index = themeOptions.indexOf(currentTheme);
    if (!["ArrowRight", "ArrowDown", "ArrowLeft", "ArrowUp", "Home", "End"].includes(event.key)) return;
    event.preventDefault();
    let nextIndex = index;
    if (["ArrowRight", "ArrowDown"].includes(event.key)) nextIndex = (index + 1) % themeOptions.length;
    if (["ArrowLeft", "ArrowUp"].includes(event.key)) nextIndex = (index - 1 + themeOptions.length) % themeOptions.length;
    if (event.key === "Home") nextIndex = 0;
    if (event.key === "End") nextIndex = themeOptions.length - 1;
    setThemeMode(themeOptions[nextIndex]);
    const nextNode = document.getElementById(`theme-${themeOptions[nextIndex]}`);
    nextNode?.focus();
  };
    ...
    <div className="page">
      ...
      <div className="settings-grid">
        <section className="panel" aria-label="Appearance settings">
          <div className="panel__header"><h2>Appearance</h2></div>
            ...
            <div className="radio-row" role="radiogroup" aria-label="Theme mode">
              {themeOptions.map((option) => (
                <button
                  ...
                  id={`theme-${option}`}
                  type="button"
                  className={cn("radio", themeMode === option && "is-selected")}
                  onClick={() => setThemeMode(option)}
                  onKeyDown={(event) => onThemeKeyDown(event, option)}
                  role="radio"
                  tabIndex={themeMode === option ? 0 : -1}
        ...
        <section className="panel" aria-label="Window and tray settings">
          <div className="panel__header"><h2>Window & Tray</h2></div>
          <div className="panel__body">
            <label className="toggle">
              <span>
              ...
              <input type="checkbox" checked={confirmTrayMinimize} onChange={(e) => setConfirmTrayMinimize(e.target.checked)} />
            </label>
          </div>
        </section>

        <section className="panel" aria-label="Updates settings">
          <div className="panel__header"><h2>Updates</h2></div>
          <div className="panel__body">
            ...
            <div className="btn-row">
              <button type="button" className="btn btn--ghost" onClick={onCheckUpdates}>Check for updates</button>
              <button type="button" className="btn btn--ghost" onClick={onDownloadUpdate} disabled={updaterStatus.state !== "available"}>Download update</button>
              <button type="button" className="btn btn--primary" onClick={onInstallUpdate} disabled={updaterStatus.state !== "downloaded"}>Install & Restart</button>
            </div>
          </div>
        </section>
      </div>
...
function HelpPage({ headingRef }) {
  return (
    <PlaceholderPage title="Help" headingRef={headingRef}>
      ...
      <p>EduLense is an AI-powered educational platform that started in K–12 education and has since expanded into higher education.</p>
      <p>It supports teachers with lesson planning, assignment generation, grading, and student performance analysis. By integrating Large Language Models (LLMs) with platforms like Moodle and Google Classroom, EduLense helps automate educational workflows while improving accessibility and ease of use.</p>
...
function SidebarItem({ active, icon, label, badge, onClick, ariaLabel }) {
  return (
    <button
      type="button"
    ...
    >
      <span className="side-item__badge" aria-hidden="true">{badge}</span>
      <span className="side-item__label">{label}</span>
      <span className="side-item__icon" aria-hidden="true">{icon}</span>
    </button>
  );
}
...
export default function App() {
  ...
  const mainRegionRef = useRef(null);
  ...
  useEffect(() => {
    if (isAuthenticated) mainHeadingRef.current?.focus();
  ...
  const handleSave = async (forceSaveAs = false) => {
    ...
    const result = forceSaveAs ? await window.desktop.file.saveAs(payload) : await window.desktop.file.save(payload);
    if (!result?.ok) {
  ...
  actionRef.current = { handleOpen, handleSave, handleNew, handleMenuCommand, navigate, navigateBack, navigateForward, setShowShortcuts };
  ...
  }, [hasHydratedState, themeMode, autoRefreshRecent, largeText, highContrast, confirmTrayMinimize, isAuthenticated]);
      ...
      <div className="auth-shell">
        ...
        <main className="auth-layout">
          <section className="auth-intro">
            ...
            <h1 ref={signInHeadingRef} tabIndex={-1}>Welcome to EduLense</h1>
            <p>EduLense is an AI-powered educational platform for lesson planning, grading, and performance analysis. Sign in to continue.</p>
          </section>
          ...
          <section className="auth-card" aria-label="Authentication actions">
            ...
            <div className="auth-actions">
              <button type="button" className="filled" onClick={() => { setIsAuthenticated(true); setStatus("Signed in"); }}>Sign In</button>
              <button type="button" className="outlined" onClick={() => { setIsAuthenticated(true); setStatus("Account created"); }}>Create Account</button>
            </div>
  ...
  const contentNode = (() => {
    switch (route) {
      case ROUTES.dashboard:
        return <DashboardPage onPrimaryAction={() => navigate(ROUTES.courses)} headingRef={mainHeadingRef} />;
      case ROUTES.courses:
        return <CoursesPage filter={courseFilter} setFilter={setCourseFilter} headingRef={mainHeadingRef} />;
      case ROUTES.notes:
        return (
          <NotesPage
            content={content}
      ...
      case ROUTES.notifications:
        return <PlaceholderPage title="Notifications" headingRef={mainHeadingRef}><p className="muted">No new notifications.</p></PlaceholderPage>;
      case ROUTES.calendar:
        return <PlaceholderPage title="Calendar" headingRef={mainHeadingRef}><p className="muted">Calendar integration is a planned feature for EduLense Desktop.</p></PlaceholderPage>;
      case ROUTES.progress:
        return <PlaceholderPage title="Progress" headingRef={mainHeadingRef}><p className="muted">Track overall course progress here.</p></PlaceholderPage>;
      case ROUTES.achievements:
        return <PlaceholderPage title="Achievements" headingRef={mainHeadingRef}><p className="muted">Badges and milestones will appear here.</p></PlaceholderPage>;
      case ROUTES.studySessions:
        return <PlaceholderPage title="Study Sessions" headingRef={mainHeadingRef}><p className="muted">Schedule and join study groups.</p></PlaceholderPage>;
      default:
        return <DashboardPage onPrimaryAction={() => navigate(ROUTES.courses)} headingRef={mainHeadingRef} />;
    }
  })();
    ...
    <div className="app-shell">
      ...
      <header className="topbar" role="banner">
        <div className="topbar__left" aria-label="App toolbar">
          <button type="button" className="btn btn--icon" aria-label="Refresh">↻</button>
          <button type="button" className="btn btn--icon" aria-label="Settings" onClick={() => navigate(ROUTES.settings)}>⚙</button>
          <div className="topbar__spacer" />
          <div className="topbar__icons" role="toolbar" aria-label="Quick actions">
            {["🕒","🔖","📘","📅","🗒","ⓘ","⇪","⬇","✎","🗑","🔗","🔍","⤢","⤡","＋"].map((c) => (
              <span key={c} className="topbar__glyph" aria-hidden="true">{c}</span>
            ))}
          </div>
        </div>
      ...
      <div className="shell">
        <main id="main-content" className="main" role="main" tabIndex={-1} ref={mainRegionRef}>
          {contentNode}
        </main>

        <nav className="sidebar" aria-label="Main navigation tabs">
          <SidebarItem active={route === ROUTES.dashboard} badge="" label="Dashboard" icon="⌂" onClick={() => navigate(ROUTES.dashboard)} />
          <SidebarItem active={route === ROUTES.courses} badge="8" label="Courses" ariaLabel="Courses, 8 items" icon="📚" onClick={() => navigate(ROUTES.courses)} />
          <SidebarItem active={route === ROUTES.calendar} badge="3" label="Calendar" ariaLabel="Calendar, 3 items" icon="📅" onClick={() => navigate(ROUTES.calendar)} />
<h2 className="sidebar__section">Workspace</h2>
          <SidebarItem active={route === ROUTES.notes} badge="" label="Notes" icon="📝" onClick={() => navigate(ROUTES.notes)} />
          <SidebarItem active={route === ROUTES.progress} badge="" label="Progress" icon="◎" onClick={() => navigate(ROUTES.progress)} />
          <SidebarItem active={route === ROUTES.achievements} badge="" label="Achievements" icon="🏅" onClick={() => navigate(ROUTES.achievements)} />
          <SidebarItem active={route === ROUTES.studySessions} badge="" label="Study Sessions" icon="🕒" onClick={() => navigate(ROUTES.studySessions)} />
          <button type="button" className="side-cta" onClick={() => setStatus("Study Groups coming soon")}> <span className="side-cta__new">New</span> Study Groups </button>

          <div className="sidebar__divider" />

          <SidebarItem active={route === ROUTES.notifications} badge="12" label="Notifications" ariaLabel="Notifications, 12 items" icon="🔔" onClick={() => navigate(ROUTES.notifications)} />
          <SidebarItem active={route === ROUTES.help} badge="" label="Help" icon="❔" onClick={() => navigate(ROUTES.help)} />
          <SidebarItem active={route === ROUTES.settings} badge="" label="Settings" icon="⚙" onClick={() => navigate(ROUTES.settings)} />
          ...
          <div className="sidebar__profile" aria-label="User profile">
            ...
            <div className="profile__avatar" aria-hidden="true">JD</div>
          </div>
        </nav>
      </div>
      ...
      <footer className="statusbar" aria-label="Status bar">
        ...
        <div className="statusbar__right" role="status" aria-live="polite">
          <span className="muted">{status}</span>
